...
class Utility {
  ...
  factory Utility() {
    if (_instance == null) {
      _instance = Utility._();
      _instance?.asyncFunc();
      _instance?.fetchUserMedicineData();
    }
    return _instance!;
  }
  ...
  UserLocal getUserData() {
    ...
    UserLocal myData = UserLocal(
        data?[0] ?? '',
        data?[1] ?? '',
        data?[2] ?? '',
        data?[3] ?? '',
        data?[4] ?? '',
        ((data?[5] ?? '').length > 0 ? new DateFormat('yyyy-MM-dd – hh:mm:ss')
            .parse(data?[5] ?? '') : DateTime.now()),
        ((data?[6] ?? '').length > 0 ? new DateFormat('yyyy-MM-dd').parse(
            data?[6] ?? '') : null),
        (data?[7] ?? '').length > 0
            ? Gender.values.firstWhere((element) =>
        element.toString() == 'Gender.' + (data?[7] ?? ''))
            : null,
        Units({
          "unit": (data?[8] ?? 'mg/dL').length > 0
              ? (data?[8] ?? 'mg/dL')
              : 'mg/dL'
        }),
        Units({
          'unit': (data?[9] ?? 'mmHg').length > 0
              ? (data?[9] ?? 'mmHg')
              : 'mmHg'
        }),
        Units({
          'unit': (data?[10] ?? 'grams').length > 0
              ? (data?[10] ?? 'grams')
              : 'grams'
        }),
        Units({
          'unit': (data?[11] ?? 'kg').length > 0 ? (data?[11] ?? 'kg') : 'kg'
        }),
        Units({
          'unit': (data?[12] ?? 'cm').length > 0 ? (data?[12] ?? 'cm') : 'cm'
        }));
    return myData;
  }
  ...
  saveUserHealthData(Map<String, dynamic> document) {
    ...
    this.prefs?.setStringList("userHealthData", arrayString)
        .whenComplete(() {});
  }

  HealthProfile getUserHealthData() {
    List<String>? data = this.prefs?.getStringList('userHealthData') as List<
        String>?;
    HealthProfile myData = HealthProfile(
        DiabetesType.values.firstWhere((element) => (data?[0] ?? "None") ==
            element.name),
        ((data?[1] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[1] ?? '') : null),
        (data?[2] ?? 'false').parseBool(),
        ((data?[3] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[3] ?? '') : null),
        (data?[4] ?? 'false').parseBool(),
        ((data?[5] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[5] ?? '') : null),
        (data?[6] ?? 'false').parseBool(),
        ((data?[7] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[7] ?? '') : null),
        (data?[8] ?? 'false').parseBool(),
        ((data?[9] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[9] ?? '') : null),
        (data?[10] ?? 'false').parseBool(),
        ((data?[11] ?? '').isNotEmpty ? DateFormat('yyyy-MM-dd').parse(
            data?[11] ?? '') : null)
    );
    return myData;
  }

  List<Medicine> usersMedicines = [];
